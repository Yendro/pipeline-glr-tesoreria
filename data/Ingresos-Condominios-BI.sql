...
WHERE Desarrollo IS NOT NULL
    ...
    AND EXTRACT (YEAR FROM DATE(FechaIngreso)) = EXTRACT(YEAR FROM CURRENT_DATE("America/Merida"))
    AND EXTRACT (MONTH FROM DATE(FechaIngreso))= EXTRACT(MONTH FROM CURRENT_DATE("America/Merida"))